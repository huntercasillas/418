...
def approximate(first, second, d):
    error = 0
...
def multiple_matching(text, patterns, d, suffix):
    ...
    for pattern in patterns:
        ...
        length = len(pattern)
        k = length // (d + 1)
        seeds = [(pattern[i * k:(i + 1) * k], i * k) for i in range(d)] + [(pattern[d * k:length], d * k)]
            ...
            while top <= bottom:
                if index >= 0:
                    ...
                    for i in range(top, bottom + 1):
                        current.add(suffix[i] - offset)
                    break
        for number in current:
            if approximate(text[number:number + length], pattern, d):
...
text, patterns, d = get_input("rosalind_ba9o.txt")
suffix = suffix_array(text)
answer = multiple_matching(text, patterns, d, suffix)
